  ...
  return <nav aria-label={en ? 'Main navigation' : '主导航'} onKeyDown={e => { if (e.key === 'Escape') { setOpen(''); e.target.closest('.blog-menu-item')?.querySelector('button')?.focus() } }}>
    {sections.map(section => {
      ...
      return <div className="blog-menu-item" key={section.path}
        onMouseEnter={() => { if (dropdown && window.matchMedia('(hover: hover)').matches) setOpen(section.path) }}
        ...
        {dropdown && <><button type="button" aria-label={`${en ? section.en : section.zh}${en ? ' categories' : '分类'}`} aria-expanded={visible} aria-controls={`menu-${section.type}`} onClick={() => setOpen(visible ? '' : section.path)}><ChevronDown size={16} /></button>
