...
const tagsOf = post => Array.isArray(post.tags) ? post.tags.filter(tag => typeof tag === 'string') : []
...
export function ArticleContent({ post, media = [], onPhoto }) {
  return <article className="blog-article">
    ...
    {post.behind_scenes && <section className="blog-prose"><h2>幕后故事 / Behind the scenes</h2><p>{text(post.behind_scenes)}</p></section>}
    {webLink(post.video_url) && <p className="blog-prose"><a href={webLink(post.video_url)} target="_blank" rel="noopener noreferrer">观看影片 / Watch video <ExternalLink size={16} /></a></p>}
    ...
    {!!tagsOf(post).length && <div className="blog-tags">{tagsOf(post).map(tag => <a key={tag} href={`/?tag=${encodeURIComponent(tag)}#articles`}>#{tag}</a>)}</div>}
  </article>
}
...
export default function Blog({ profile, lang, setLang }) {
  ...
  const view = pathname === '/activities' ? 'activities' : pathname === '/bookroom' ? 'bookroom' : pathname === '/about' ? 'about' : 'home'
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        let pictures = current ? await allRows(() => supabase.from('blog_media').select('*').eq('post_id', current.id).order('position').order('id')) : []
        try {
          const publishedAlbums = await allRows(() => supabase.from('blog_albums').select('*').eq('status', 'published').order('content_year', { ascending: false }).order('id'))
          if (active) setAlbums(publishedAlbums)
          if (current) {
            const joins = await rows(supabase.from('blog_post_albums').select('album_id').eq('post_id', current.id))
            const ids = publishedAlbums.filter(album => joins.some(join => join.album_id === album.id) || album.event_id === current.id || (current.event_id && album.event_id === current.event_id)).map(album => album.id)
            for (let i = 0; i < ids.length; i += 50) {
              pictures.push(...await allRows(() => supabase.from('blog_media').select('*').in('album_id', ids.slice(i, i + 50)).order('position').order('id')))
            }
            const publicLinks = await rows(supabase.from('blog_links').select('*').eq('post_id', current.id).eq('visibility', 'public').order('position').order('id'))
            const memberLinks = activeMember ? await rows(supabase.from('blog_links').select('*').eq('post_id', current.id).eq('visibility', 'member').order('position').order('id')) : []
            if (active) setLinks([...publicLinks, ...memberLinks])
          }
        } catch { if (active) setError('extras') }
        if (active) setMedia([...new Map(pictures.map(photo => [photo.id || photo.path, photo])).values()])
  ...
  async function openAlbum(album) {
    if (albumBusy) return
    albumTriggerRef.current = document.activeElement
    setAlbumBusy(true)
    try {
      const photos = await allRows(() => supabase.from('blog_media').select('*').eq('album_id', album.id).order('position').order('id'))
      if (photos.length) setLightbox({ photos, index: 0, title: album.title })
      else { albumTriggerRef.current = null; setError('album') }
    } catch { albumTriggerRef.current = null; setError('extras') }
  ...
  const scopedPosts = posts.filter(item => view === 'activities' ? item.content_type === 'event' : view === 'bookroom' ? ['article', 'publication'].includes(item.content_type || 'article') : true)
  const filtered = scopedPosts.filter(item => (!category || item.category_id === category) && (!year || yearOf(item) === year) && (!tag || tagsOf(item).includes(tag)) && `${item.title} ${item.summary || ''} ${tagsOf(item).join(' ')}`.toLowerCase().includes(search.toLowerCase().trim()))
  ...
  const tags = [...new Set(scopedPosts.flatMap(tagsOf))].sort()
  const manual = Array.isArray(post?.related_ids) ? post.related_ids : []
  const score = item => (post?.event_id && (item.id === post.event_id || item.event_id === post.event_id) || item.event_id === post?.id ? 5 : 0) + (post?.category_id && item.category_id === post.category_id ? 3 : 0) + tagsOf(item).filter(value => tagsOf(post || {}).includes(value)).length
  ...
  const card = item => <a className="blog-post" key={item.id} href={blogPath(item.slug)}><BlogImage path={item.cover_path} alt={item.title} loading="lazy" /><div><p className="blog-eyebrow">{categories.find(cat => cat.id === item.category_id)?.name || t('学会记录', 'Club journal')}{item.is_sticky ? t(' · 置顶', ' · Pinned') : ''}</p><h3>{item.title}</h3><time>{dateOf(item).slice(0, 10) || yearOf(item)}</time><p>{item.summary}</p><span>{t('阅读全文', 'Read story')}<ArrowRight size={16} /></span></div></a>
  const aboutNotes = text(content.about_notes || settings.about || settings.intro).split(/\n\s*\n/).map(note => note.trim()).filter(Boolean)
  const archiveYears = new Set([...posts.map(yearOf), ...albums.map(album => String(album.content_year || ''))].filter(Boolean)).size
  const about = <section className="blog-community" id="about">
    <div className="blog-community-intro"><h2>{override('about_title', settings.title)}</h2><p>{settings.intro}</p>
      ...
      <dl className="blog-community-counts">{[[posts.length, t('公开文章', 'Public stories')], [albums.length, t('公开相册', 'Public albums')], [archiveYears, t('记录年份', 'Years recorded')]].map(([value, label]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>
    </div>
  ...
  return <div className="club-blog blog-public">
    ...
    <main className="blog-main" id="blog-content">
      ...
      {loading ? <div className="blog-skeleton" role="status" aria-label={t('载入中', 'Loading')} /> : slug ? post ? <>
        <a className="blog-back" href="/activities"><ArrowLeft size={16} /> {t('返回活动记录', 'Back to activities')}</a><ArticleContent post={post} media={media} onPhoto={index => setLightbox({ photos: media, index, title: post.title })} />
      ...
      </> : <section className="blog-empty"><h1>{t('文章不存在或尚未公开', 'Article unavailable')}</h1><a href="/">{t('返回首页', 'Back to home')}</a></section> : view === 'about' ? about : <>
        <section id="articles"><div className="blog-section-heading"><div><p className="blog-eyebrow">{view === 'bookroom' ? 'THE READING ROOM' : 'OUR STORIES'}</p><h2>{view === 'home' ? override('latest_title', t('最新活动', 'Latest stories')) : view === 'activities' ? t('活动记录', 'Activity archive') : t('书坊', 'Bookroom')}</h2></div>{view === 'home' && <a href="/activities">{t('查看全部活动', 'All activities')} <ArrowRight size={16} /></a>}</div>
          <div className="blog-filters"><label className="blog-search"><Search size={18} /><input aria-label={t('搜索文章', 'Search articles')} placeholder={t('搜索文章、标签', 'Search stories and tags')} value={search} onChange={event => { setSearch(event.target.value); setPage(1) }} /></label><select aria-label={t('年份', 'Year')} value={year} onChange={event => { setYear(event.target.value); setPage(1) }}><option value="">{t('所有年份', 'All years')}</option>{years.map(value => <option key={value}>{value}</option>)}</select><select aria-label={t('排序', 'Sort order')} value={sort} onChange={event => { setSort(event.target.value); setPage(1) }}><option value="desc">{t('由新至旧', 'Newest first')}</option><option value="asc">{t('由旧至新', 'Oldest first')}</option></select><select aria-label={t('标签', 'Tag')} value={tag} onChange={event => { setTag(event.target.value); setPage(1) }}><option value="">{t('所有标签', 'All tags')}</option>{[...new Set([...tags, ...(tag ? [tag] : [])])].map(value => <option key={value} value={value}>#{value}</option>)}</select></div>
          <div className="blog-categories"><button aria-pressed={!category} onClick={() => { setCategory(''); setPage(1) }}>{t('全部', 'All')}</button>{categories.map(item => <button key={item.id} aria-pressed={category === item.id} onClick={() => { setCategory(item.id); setPage(1) }}>{item.name}</button>)}</div>
        ...
        {view === 'home' && <><section className="blog-public-section"><div className="blog-section-heading"><div><p className="blog-eyebrow">EDITOR’S PICKS</p><h2>{override('featured_title', t('精选故事', 'Featured stories'))}</h2></div><BookOpen size={28} /></div><div className="blog-post-grid">{posts.filter(item => item.featured).slice(0, 4).map(card)}</div>{!posts.some(item => item.featured) && <p className="blog-empty">{t('精选故事即将更新。', 'Featured stories are coming soon.')}</p>}</section><section className="blog-public-section blog-album-section" id="albums"><div className="blog-section-heading"><div><p className="blog-eyebrow">MOMENTS WORTH REMEMBERING</p><h2>{override('albums_title', t('留下来的，不只是照片', 'More than photographs'))}</h2></div><Camera size={28} /></div><div className="blog-album-grid">{albums.filter(album => !year || String(album.content_year) === year).map(album => <button key={album.id} className="blog-album" disabled={albumBusy} onClick={() => openAlbum(album)}><BlogImage path={album.cover_path} alt={album.title} loading="lazy" /><span><strong>{album.title}</strong><small>{album.content_year}</small></span></button>)}</div>{!albums.length && <p className="blog-empty">{t('相册即将更新。', 'Albums are coming soon.')}</p>}</section>{about}</>}
      </>}
    </main><footer className="blog-footer"><div><strong>{settings.title}</strong><p>{settings.contact || settings.subtitle}</p></div><div><p>{t('记录现在，传承以后。', 'Stories today. Memories for tomorrow.')}</p><span>© {new Date().getFullYear()} CHMS1 Chinese Language Club</span></div><a href="/#/">{t('会员系统', 'Member system')} <ArrowRight size={16} /></a></footer>
