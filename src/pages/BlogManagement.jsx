...
import { LayoutDashboard, Files, Images, Tags, Trash2, Settings, ChartColumn, Plus, Pencil, RotateCcw, Search, RefreshCw, Archive, LockKeyhole, Pin, ArrowLeft, ExternalLink } from 'lucide-react'
import { supabase } from '../supabaseClient'
import { canManageBlog, emptyArticle } from '../utils/blog'
import { StudioPostEditor, StudioAlbumEditor, StudioTaxonomy, StudioSettings } from './BlogStudioEditors'
...
const labels = { article: '文章', event: '活动', publication: '出版物', notice: '公告', draft: '草稿', review: '待审核', published: '已发布', scheduled: '定时发布', hidden: '已隐藏', trash: '回收站' }
const navigation = [['dashboard', LayoutDashboard, '总览', 'Dashboard'], ['content', Files, '内容管理', 'Content'], ['albums', Images, '独立相册', 'Albums'], ['taxonomy', Tags, '分类与标签', 'Taxonomy'], ['analytics', ChartColumn, '访问统计', 'Analytics'], ['trash', Trash2, '回收站', 'Recycle bin'], ['settings', Settings, '网站设置', 'Site settings']]
...
export default function BlogManagement({ profile, lang = 'zh' }) {
  ...
  const usesYear = ['dashboard', 'content', 'albums', 'trash'].includes(tab)
  ...
  async function load() {
    ...
    const albums = await allRows('blog_albums')
  ...
  function navigate(next) { if (busy || !discard()) return; setEditor(null); setDirty(false); setTab(next); setFilters({ search: '', category: '', status: '', tag: '', type: '', sort: 'updated', quick: 'all' }); setPage(1); setError(''); setMessage('') }
  ...
  async function open(row, kind = 'post') {
    ...
    await run(async () => {
      ...
      const joins = row.id && kind === 'post' ? await checked(supabase.from('blog_post_albums').select('album_id').eq('post_id', row.id)) : []
    ...
    await run(async () => {
      ...
      if (permanent) {
        ...
        if (kind === 'album') {
          ...
          const joins = await checked(supabase.from('blog_post_albums').select('album_id').eq('post_id', row.id))
  ...
  rows = rows.filter(row => (!filters.search || `${row.title} ${row.summary || ''} ${row.description || ''} ${row.slug || ''}`.toLowerCase().includes(filters.search.toLowerCase())) && (!filters.status || row.status === filters.status) && (!filters.category || row.category_id === filters.category) && (!filters.tag || row.tags?.includes(filters.tag)) && (!filters.type || row.content_type === filters.type) && (filters.quick !== 'featured' || row.featured) && (filters.quick !== 'sticky' || row.is_sticky) && (filters.quick !== 'mine' || row.created_by === profile?.id))
  ...
  return <div className="bs-layout">
    <aside className="bs-sidebar"><div className="bs-sidebar-title">{t('华文学会后台', 'CLC Blog Studio')}</div><nav aria-label={t('后台导航', 'Studio navigation')}>{navigation.map(([id, Icon, zh, english]) => <button key={id} className={tab === id ? 'active' : ''} aria-current={tab === id ? 'page' : undefined} disabled={busy} onClick={() => tab !== id && navigate(id)}><Icon size={18} />{t(zh, english)}</button>)}</nav><a href="/" target="_blank" rel="noreferrer"><ExternalLink size={16} />{t('公开网站', 'Public website')}</a></aside>
    <main className="bs-workspace"><header className="bs-heading"><div><p>CLC_sys / {t('网站管理', 'Website administration')}</p><h1>{t(selectedNav[2], selectedNav[3])}</h1></div><div className="bs-actions">{usesYear && <label className="bs-year">{t('年份', 'Year')}<select aria-label={t('管理年份', 'Management year')} disabled={busy} value={year} onChange={e => { if (discard()) { setYear(e.target.value); setEditor(null); setDirty(false); setPage(1) } }}><option value="">{t('全部年份', 'All years')}</option>{yearOptions.map(y => <option key={y} value={y}>{y}{archived(y) ? t(' · 已归档', ' · Archived') : ''}</option>)}</select></label>}<button title={t('刷新', 'Refresh')} aria-label={t('刷新', 'Refresh')} disabled={busy || dirty} onClick={() => run(load, '')}><RefreshCw size={17} /></button></div></header>
      ...
      {usesYear && year && archived(year) && <p className="bs-alert"><LockKeyhole size={16} />{t('此年份已归档，内容与相册为只读。', 'This year is archived. Content and albums are read-only.')}</p>}
      {loading ? <p className="bs-empty" role="status">{t('正在载入后台…', 'Loading studio…')}</p> : editor ? <><div className="bs-toolbar"><button disabled={busy} onClick={() => { if (discard()) { setEditor(null); setDirty(false) } }}><ArrowLeft size={17} />{t('返回列表', 'Back to list')}</button>{dirty && <span className="bs-muted">{t('尚未保存', 'Unsaved changes')}</span>}</div>{editor.kind === 'post' ? <StudioPostEditor key={editor.row.id || 'new-post'} initial={editor} data={data} busy={busy} run={run} reload={load} setDirty={setDirty} en={en} archived={archived} onSaved={row => setEditor(e => ({ ...e, row }))} /> : <StudioAlbumEditor key={editor.row.id || 'new-album'} initial={editor} data={data} busy={busy} run={run} reload={load} setDirty={setDirty} en={en} archived={archived} onSaved={row => setEditor(e => ({ ...e, row }))} />}</> : <>
        {tab === 'dashboard' && <><div className="bs-stats">{[[t('内容总数', 'Total content'), activePosts.length], [t('已发布', 'Published'), activePosts.filter(p => p.status === 'published').length], [t('待审核', 'In review'), activePosts.filter(p => p.status === 'review').length], [t('独立相册', 'Albums'), activeAlbums.length]].map(([title, value]) => <div key={title}><span>{title}</span><strong>{value}</strong></div>)}</div><section className="bs-section"><div className="bs-section-heading"><h2>{t('最近编辑', 'Recent edits')}</h2><button onClick={() => navigate('content')}>{t('全部内容', 'All content')}</button></div><ContentTable rows={[...activePosts].sort((a, b) => new Date(b.updated_at) - new Date(a.updated_at)).slice(0, 8).map(p => ({ ...p, kind: 'post' }))} categories={data.categories} t={t} label={label} busy={busy} archived={archived} open={open} changeState={changeState} /></section><section className="bs-section"><h2>{t('年份管理', 'Year archive')}</h2><form className="bs-inline" onSubmit={e => { e.preventDefault(); const value = Number(new FormData(e.currentTarget).get('year')); run(async () => { await checked(supabase.from('blog_years').upsert({ year: value }, { onConflict: 'year', ignoreDuplicates: true })); await load(); setYear(String(value)) }) }}><label>{t('新增年份', 'Add year')}<input name="year" type="number" min="1900" max="2200" defaultValue={currentYear + 1} required /></label><button disabled={busy}><Plus size={16} />{t('新增', 'Add')}</button></form><div className="bs-year-list">{yearOptions.map(y => <div key={y}><strong>{y}</strong><span>{archived(y) ? t('已归档', 'Archived') : t('开放编辑', 'Open')}</span><button disabled={busy || !canArchive} title={t('仅老师与会长可更改归档状态', 'Only teachers and the president can change archive status')} onClick={() => { if (window.confirm(archived(y) ? t(`重新开放 ${y} 年？`, `Reopen ${y}?`) : t(`归档 ${y} 年？内容、相册及照片将锁定。`, `Archive ${y}? Content, albums and media will be locked.`))) run(async () => { await checked(supabase.from('blog_years').upsert({ year: y, is_archived: !archived(y) }, { onConflict: 'year' })); await load() }) }}><Archive size={16} />{archived(y) ? t('重新开放', 'Reopen') : t('归档', 'Archive')}</button></div>)}</div></section></>}
        {['content', 'albums', 'trash'].includes(tab) && <><div className="bs-section-heading"><h2>{tab === 'content' ? t('内容列表', 'Content library') : tab === 'albums' ? t('相册列表', 'Album library') : t('已删除的内容与相册', 'Deleted content and albums')} <span className="bs-count">{rows.length}</span></h2>{tab !== 'trash' && <button className="bs-primary" disabled={busy || (year && archived(year))} onClick={() => create(tab === 'albums' ? 'album' : 'post')}><Plus size={17} />{tab === 'albums' ? t('新增相册', 'New album') : t('新增内容', 'New content')}</button>}</div>
          {tab === 'content' && <div className="bs-quick">{[['all', '全部', 'All'], ['featured', '首页精选', 'Featured'], ['sticky', '置顶', 'Sticky'], ['mine', '我创建的', 'Created by me']].map(([id, zh, english]) => <button key={id} aria-pressed={filters.quick === id} onClick={() => filter('quick', id)}>{t(zh, english)}</button>)}</div>}
          <div className="bs-filters"><label className="bs-search"><Search size={17} /><input aria-label={t('搜索内容', 'Search content')} placeholder={t('搜索标题、摘要…', 'Search title, summary…')} value={filters.search} onChange={e => filter('search', e.target.value)} /></label>{tab === 'content' && <><select aria-label={t('内容类型', 'Content type')} value={filters.type} onChange={e => filter('type', e.target.value)}><option value="">{t('全部类型', 'All types')}</option>{types.map(v => <option key={v} value={v}>{label(v)}</option>)}</select><select aria-label={t('分类', 'Category')} value={filters.category} onChange={e => filter('category', e.target.value)}><option value="">{t('全部分类', 'All categories')}</option>{data.categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select><select aria-label={t('标签', 'Tag')} value={filters.tag} onChange={e => filter('tag', e.target.value)}><option value="">{t('全部标签', 'All tags')}</option>{data.tags.map(c => <option key={c.id} value={c.name}>{c.group_name ? `${c.group_name} / ` : ''}{c.name}</option>)}</select></>}{tab !== 'trash' && <select aria-label={t('状态', 'Status')} value={filters.status} onChange={e => filter('status', e.target.value)}><option value="">{t('全部状态', 'All statuses')}</option>{(tab === 'albums' ? ['draft', 'published', 'hidden'] : statuses).map(v => <option key={v} value={v}>{label(v)}</option>)}</select>}<select aria-label={t('排序', 'Sort')} value={filters.sort} onChange={e => filter('sort', e.target.value)}>{[['updated', '最近编辑', 'Recently edited'], ['title', '标题', 'Title'], ['oldest', '最早创建', 'Oldest first'], ['event', '活动日期', 'Event date']].map(([v, zh, english]) => <option key={v} value={v}>{t(zh, english)}</option>)}</select></div>
        ...
        {tab === 'taxonomy' && <StudioTaxonomy data={data} busy={busy} run={run} reload={load} en={en} setDirty={setDirty} />}
