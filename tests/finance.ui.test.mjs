...
try {
  for (const [width,height,lang] of [[1440,1000,'zh'],[390,844,'zh'],[390,844,'en']]) {
    ...
    if (width===1440 && process.env.FINANCE_PRINT_TEST==='1') {
      ...
      await page.locator('.finance-table tbody').evaluate(body => { while(body.children.length>13) body.children[12].remove() })
    }
    ...
    assert.deepEqual(await page.locator('tfoot td').allTextContents(),['563.30','563.30'])
