...
export function renderBlogHtml(template, site, posts, media, slug, origin, view = 'home', links = []) {
  ...
  const names = { home: '首页', activities: '活动记录', bookroom: '书坊', about: '关于我们' }
  ...
  const structured = post
    ? { '@context': 'https://schema.org', '@type': 'BlogPosting', headline: post.title, description, datePublished: post.published_at, dateModified: post.updated_at, mainEntityOfPage: canonical, image: cover ? photoUrl(cover) : undefined, author: { '@type': 'Organization', name: site.title } }
  ...
  return template.replace(/<title>[\s\S]*?<\/title>/, '').replace(/<meta name="description"[^>]*>/, '')
    ...
    .replace('<div id="root"></div>', () => `<div id="root"><div class="club-blog">${nav}${body}<footer class="blog-footer">${e(site.contact)}</footer></div></div>`)
}
